...
const siteTitle = process.env.SITE_TITLE || 'IOQ3 SERVER';
const siteSubtitle = process.env.SITE_SUBTITLE || '';
const siteDomain = process.env.SITE_DOMAIN || 'localhost';
const siteGithubUser = process.env.SITE_GITHUB_USER || '';
      ...
      return `
        <tr style="background:${zebra};">
          <td style="padding:12px 16px; border:1px solid #333;">
            ...
            <div style="font-size:12px; color:#aaa;">${status.host.replace(/^quake[0-9]/, siteDomain)}:${status.displayPort || status.port}</div>
          </td>
...
<div class="wrapper">
  ...
  <h1>${siteTitle}</h1>
  ${siteSubtitle ? `<div class="subtitle">${siteSubtitle}</div>` : ''}
  <table>
    <tbody>
      ${rows}
    </tbody>
  ...
  <div class="footer">© ${year}${siteGithubUser ? ` <a href="https://github.com/${siteGithubUser}" style="color:#ffdd57; text-decoration:none;">${siteGithubUser}</a>` : ''} — frag responsibly${siteGithubUser ? ` <a class="github-link" href="https://github.com/${siteGithubUser}/ioq3-server" target="_blank" rel="noopener noreferrer"><img src="/github-mark.svg" alt="GitHub project" /></a>` : ''}</div>
</body>
<script>
